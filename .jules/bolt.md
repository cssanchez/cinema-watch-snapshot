...
## 2024-05-18 - [Optimizing find max in Maps]
**Learning:** To avoid unnecessary garbage collection overhead and intermediate array allocations when processing Iterable objects (like Map iterators), replace `Array.from(map.keys()).reduce(...)` with a direct `for...of` loop over `map.entries()`. Allocating arrays just to iterate map keys is an O(N) allocation waste.
**Action:** Iterate directly using `for...of map.entries()` when finding maximum or optimum elements in maps.
