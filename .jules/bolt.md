...
## 2025-07-01 - [Map Iteration Memory Allocation]
**Learning:** In frontend statistics rendering loops (e.g. `buildFrontInsights`), executing `Array.from(map.keys()).reduce(...)` allocates a new array, taking significantly longer and triggering unnecessary garbage collection compared to standard iteration.
**Action:** To avoid unnecessary garbage collection overhead and intermediate array allocations when processing Iterable objects (like Map iterators), replace `Array.from(map.keys()).reduce(...)` with a direct `for...of` loop over `map.entries()`.
