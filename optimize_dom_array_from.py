...
def process_file(file_path):
    ...
    # 5. initFrontBoards - optimize multi-pass Array.from + map + filter to single-pass Set population
    pattern5 = re.compile(
    ...
    def repl5(match):
        indent = match.group(1)
        return (
            f"{indent}// ⚡ Bolt Optimization: Replace Array.from(NodeList).map().filter() with a single-pass loop\n"
    ...
    # 6. initFrontAdvanced - optimize multi-pass Array.from + map + filter to single-pass Set population
    pattern6 = re.compile(
        r"([ \t]*)const locationKeys = new Set\(\s*"
    ...
    def repl6(match):
        ...
        return (
            ...
            f"{indent}  const val = forms[i] instanceof HTMLFormElement ? String(forms[i].dataset.frontLocationKey || '').trim() : '';\n"
            f"{indent}  if (val) locationKeys.add(val);\n"
            f"{indent}}}"
        )
