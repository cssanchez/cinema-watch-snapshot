...
def process_file(file_path):
    ...
    # 5. initFrontBoards locationKeys creation
    pattern5 = re.compile(
    ...
    # 6. syncActiveQuickBoardPresets locationKeys creation
    pattern6 = re.compile(
        r"([ \t]*)const locationKeys = new Set\(\s*"
    ...
    # 7. Array.from(field.options).some(...) -> Array.prototype.some.call(field.options, ...)
    pattern7 = re.compile(
        r"Array\.from\(field\.options\)\.some\(",
        re.MULTILINE
    )

    def repl7(match):
        return "Array.prototype.some.call(field.options, "
