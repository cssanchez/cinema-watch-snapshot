...
def process_file(file_path):
    ...
    def repl5(match):
        ...
        return (
            ...
            f"{indent}const locations = root.querySelectorAll('[data-front-board-location]');\n"
            f"{indent}for (let i = 0; i < locations.length; i++) {{\n"
            f"{indent}  const val = locations[i].getAttribute('data-front-board-location');\n"
    ...
    # 6. syncActiveQuickBoardPresets replacement
    pattern6 = re.compile(
        r"([ \t]*)const locationKeys = new Set\(\s*"
