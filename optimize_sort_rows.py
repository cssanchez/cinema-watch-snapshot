...
def process_file(file_path):
    ...
    def repl_sort_rows(match):
        indent = match.group(1)
        return (
            f"// ⚡ Bolt Optimization: Replace slow localeCompare and string concatenation with sequential inequality checks\n"
            ...
            f"{indent}      const lMovie = left.movie_title || '';\n"
            f"{indent}      const rMovie = right.movie_title || '';\n"
            f"{indent}      if (lMovie !== rMovie) return lMovie < rMovie ? -1 : 1;\n"
            f"{indent}      return 0;\n"
            f"{indent}    }});\n"
    ...
    content = pattern_sort_rows.sub(repl_sort_rows, content)
