...
def test_replaces_initFrontBoards(temp_docs_dir):
    ...
    assert "Array.from(root.querySelectorAll" not in content
    assert "const locationKeys = new Set();" in content
    assert "const boardLocations = root.querySelectorAll('[data-front-board-location]');" in content
    assert "for (let i = 0; i < boardLocations.length; i++)" in content

    # Test idempotency
    optimize_dom_array_from.process_file(test_html)
    content2 = test_html.read_text()
...
def test_replaces_syncActiveQuickBoardPresets(temp_docs_dir):
    ...
    assert "Array.from(root.querySelectorAll" not in content
    assert "const locationKeys = new Set();" in content
    assert "const advancedForms = root.querySelectorAll('form[data-front-advanced-form=\"true\"]');" in content
    assert "for (let i = 0; i < advancedForms.length; i++)" in content
