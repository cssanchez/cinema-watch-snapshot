...
def transform_12_add_aria_live(content: str) -> str:
    """
    Add aria-live="polite" and role="status" to dynamic results container and empty states.
    """
    import re

    # 1. Add to the dynamic results container
    container_target = '<div class="front-screening-groups" data-front-advanced-results>'
    container_replacement = '<div class="front-screening-groups" data-front-advanced-results aria-live="polite" role="status">'
    if container_target in content:
        content = content.replace(container_target, container_replacement)

    # 2. Add to empty state messages
    content = re.sub(
        r'<div\s+class="empty"(?!\s+aria-live="polite")',
