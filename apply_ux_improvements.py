...
RE_SOLD_PERCENTAGE = re.compile(r'(\d+)%\s*sold')
RE_SCREENING_ROW = re.compile(r'<div class="front-screening-row">[^<]*(?:<[^/][^>]*>(?:[^<]|<[^d][^>]*>)*)*(?:</[^>]*>)*?</div>', flags=re.DOTALL)

RE_DATE_GROUP_SECTION = re.compile(r'<section class="front-date-group">(?:[^<]|<(?!/section))*</section>', flags=re.DOTALL)
RE_FRONT_SCREENING_ROW_OPEN = re.compile(r'<div class="front-screening-row"')
...
def transform_1_demote_freshness(content: str) -> str:
    ...
    - Reduce padding
    """
    replacement_font = r'\1font-family: var(--ui-font)\2font-size: 0.9rem'
    content = RE_FRESHNESS_FONT.sub(replacement_font, content)
...
def extract_sold_percentage(screening_text: str) -> float:
    ...
    match = RE_SOLD_PERCENTAGE.search(screening_text)
    if match:
...
def transform_4_occupancy_highlight(content: str) -> str:
    ...
    def replace_row(match):
        ...
        sold_match = RE_SOLD_PERCENTAGE.search(row_html)
        if sold_match:
    ...
    content = RE_SCREENING_ROW.sub(replace_row, content)
...
def transform_5_count_format(content: str) -> str:
    ...
    def replace_shown_count(match):
        ...
        row_count = len(RE_FRONT_SCREENING_ROW_OPEN.findall(section))
        
        # More targeted approach: find and replace within the section
        section = RE_SHOWN_COUNT.sub(
            lambda m: m.group(1) + m.group(2) + f' of {row_count}' + m.group(3),
            section
        )
    ...
    content = RE_DATE_GROUP_SECTION.sub(replace_shown_count, content)
