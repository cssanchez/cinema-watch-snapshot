...
def transform_12_add_aria_live_to_results(content: str) -> str:
    """
    Add aria-live attributes to dynamically updated screening result containers
    and empty state messages so screen readers announce changes automatically.
    """
    if '<div class="front-screening-groups" data-front-advanced-results aria-live="polite" role="status"></div>' in content:
        return content

    # 1. Update results containers
    ...
    content = content.replace(
        ...
        '<div class="empty" aria-live="polite" role="status" data-i18n-source>'
    )
